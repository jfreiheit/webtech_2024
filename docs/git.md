...
Wir können Git nun verwenden und betrachten zunächst, wie [Git als Versionierungswerkzeug](https://git-scm.com/video/what-is-version-control) verwendet werden kann und schauen uns danach an, wie es als Kollaborationswerkzeug (falls Sie zu mehreren an einem gemeinsamen Projekt arbeiten) verwendet wird. 
...
```bash
[main 44ed3a8] in A.java Zeile 1 hinzugefuegt
...
1. arbeiten wir jetzt auf einem anderen Branch, als dem **main**-Branch, nämlich dem **tryB**-Branch und
...
- auf dem **main**-Branch weitergearbeitet und dort ein Commit `9c869b6` (`in A.java Zeile 2 hinzugefuegt`) erstellt und
...
```bash
git remote add origin https://github.com/jfreiheit/WebTechCode.git
```
...
können wir ebenfalls ausführen. Wir wissen bereits, dass wir damit einen neuen Branch erstellen, nämlich `main` (sollten Sie bereits im `main`-Branch sein, ändert sich durch den Befehl nichts). Die Option `-M` steht für `--move --force`. Damit bestimmen wir, dass wir einen `main`-Branch erstellen, selbst wenn dieser bereits existierte. Siehe auch die Dokumentation für [git branch](https://git-scm.com/docs/git-branch).
